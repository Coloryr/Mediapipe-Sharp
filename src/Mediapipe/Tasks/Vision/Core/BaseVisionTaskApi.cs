...
public class BaseVisionTaskApi : IDisposable
{
    ...
    protected static void ConfigureNormalizedRect(NormalizedRect target, ImageProcessingOptions? options, Image image, bool roiAllowed = true)
    {
        ResetNormalizedRect(target);

        if (options is not ImageProcessingOptions optionsValue)
        {
            return;
        }
